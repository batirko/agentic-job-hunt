#!/usr/bin/env node
/**
 * analyze-patterns.mjs — Rejection Pattern Detector for agentic-job-hunt
 *
...
import { normalizeStatus, classifyOutcome } from './states-core.mjs';
import { parseTrackerAll, resolveTrackerPath, resolveReportPath } from './tracker-core.mjs';
...
// Outcome buckets produced by classifyOutcome() in states-core.mjs.
// no_response (👻 ignored) is deliberately separate from negative: a company
// that never answered is not a company that said no.
const OUTCOMES = ['positive', 'negative', 'no_response', 'self_filtered', 'pending'];
const emptyOutcomeCounts = () => Object.fromEntries(OUTCOMES.map(o => [o, 0]));
const emptyOutcomeLists = () => Object.fromEntries(OUTCOMES.map(o => [o, []]));
...
// Status normalization and tracker row parsing live in states-core.mjs /
// tracker-core.mjs so the emoji mapping stays sourced from templates/states.yml.
...
function parseReport(reportPath) {
  ...
  if (gapTableMatch) {
    ...
    for (const row of gapRows) {
      const cols = splitRow(row).filter(Boolean);
      if (cols.length >= 2) {
        report.gaps.push({
          description: cols[0],
...
// --- Main analysis ---
function analyze() {
  // Both files: the archive holds the rejections and the silences, which is
  // most of what there is to learn from. Reading only the live tracker would
  // report on the applications still in flight and call it a pattern.
  const entries = parseTrackerAll(CAREER_OPS);
  ...
  const enriched = entries.map(e => {
    const reportPath = resolveReportPath(e.report, e.file || APPS_FILE);
    const reportData = reportPath ? parseReport(reportPath) : null;
    const outcome = classifyOutcome(e.status);

    // Fallback: if report didn't have Remote field, try the location and
    // reasoning columns, which carry the same signal in the current schema.
    const remoteSource = reportData?.remote || e.location || e.notes || '';
  ...
  // The tracker carries three scores: fit ("do I qualify?"), odds ("will they
  // pick me?") and priority ("should I focus here?"). Report all three so it
  // is visible which one actually separates outcomes.
  const collect = (key) => {
    const byOutcome = emptyOutcomeLists();
    for (const e of enriched) {
      if (e[key] > 0) byOutcome[e.outcome].push(e[key]);
    }
    return byOutcome;
  };

  const scoresByOutcome = collect('fit');
  const oddsByOutcome = collect('odds');
  const priorityByOutcome = collect('priority');
  ...
  for (const outcome of OUTCOMES) {
    scoreComparison[outcome] = {
      ...scoreStats(scoresByOutcome[outcome]),
      odds: scoreStats(oddsByOutcome[outcome]),
      priority: scoreStats(priorityByOutcome[outcome]),
    };
  }
  ...
  for (const e of enriched) {
    ...
    if (!archetypeMap.has(arch)) archetypeMap.set(arch, { total: 0, ...emptyOutcomeCounts() });
  ...
  for (const e of enriched) {
    ...
    if (!remoteMap.has(policy)) remoteMap.set(policy, { total: 0, ...emptyOutcomeCounts() });
  ...
  for (const e of enriched) {
    ...
    if (!sizeMap.has(size)) sizeMap.set(size, { total: 0, ...emptyOutcomeCounts() });
  ...
  // --- Tech stack gaps (from every outcome that did not move forward) ---
  // no_response counts here too: a ghosted application is a failed one, and
  // silence is now the most common failure mode, so excluding it would leave
  // the gap analysis nearly blind.
  const FAILED_OUTCOMES = ['negative', 'no_response', 'self_filtered'];
  const stackGapCounts = new Map();
  for (const e of enriched) {
    if (!FAILED_OUTCOMES.includes(e.outcome)) continue;
    if (!e.report?.gaps) continue;
    for (const gap of e.report.gaps) {
...
function printSummary(result) {
  ...
  const funnelOrder = ['evaluated', 'applied', 'responded', 'interview', 'offer', 'rejected', 'ignored', 'discarded', 'skip'];
  ...
  // Score comparison
  console.log('\nSCORE BY OUTCOME (avg)');
  console.log('-'.repeat(40));
  console.log('  ' + 'outcome'.padEnd(15) + 'fit'.padEnd(8) + 'odds'.padEnd(8) + 'priority'.padEnd(10) + 'n');
  for (const [group, stats] of Object.entries(scoreComparison)) {
    if (stats.count > 0) {
      console.log(
        ...
        group.padEnd(15) +
        `${stats.avg}`.padEnd(8) +
        `${stats.odds.avg}`.padEnd(8) +
        `${stats.priority.avg}`.padEnd(10) +
        stats.count
      );
    }
  }
