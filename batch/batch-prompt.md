# agentic-job-hunt Batch Worker — Evaluación Completa + PDF + Tracker Line
...
**PDF:** agentic-job-hunt/output/cv-candidate-{company-slug}-{{DATE}}.pdf
**Batch ID:** {{ID}}
...
Formato TSV (una sola línea, sin header, 13 columnas tab-separated):
```
{next_num}\t{{DATE}}\t{empresa}\t{rol}\t{fit}/5\t{odds}/5\t{priority}/5\t{status}\t{pdf_emoji}\t[{{REPORT_NUM}}](../reports/{{REPORT_NUM}}-{company-slug}-{{DATE}}.md)\t{location}\t{nota_1_frase}\t{url}
```
...
|---|-------|------|---------|------------|
| 1 | num | int | `647` | Report number (3 dígitos, zero-padded) |
| 2 | date | YYYY-MM-DD | `2026-03-14` | Fecha de evaluación |
| 3 | company | string | `Datadog` | Nombre corto de empresa |
| 4 | role | string | `Staff PM` | Título del rol |
| 5 | fit | X.X/5 | `4.2/5` | "Do I qualify?" |
| 6 | odds | X.X/5 | `3.8/5` | "Will they pick me?" |
| 7 | priority | X.X/5 | `4.5/5` | "Should I focus here?" |
| 8 | status | emoji o vacío | `✅` (vacío = evaluated) | Emoji canónico o vacío (ver states.yml). NUNCA texto |
| 9 | output | emoji | `[📁](../output/slug)` o `-` | Link a output folder |
| 10 | report | md link | `[007](../reports/007-...)` | Link al report (`../` obligatorio: applications.md vive en `data/`) |
| 11 | location | string | `Berlin, Germany` | Ubicación del rol |
| 12 | notes | string | `Strong DevEx match...` | Resumen 1 frase |
| 13 | url | string | `https://...` | URL de la oferta original |

**Estados canónicos válidos (emoji, ver `templates/states.yml`):** vacío = Evaluated, `✅` Applied, `📬` Responded, `🎯` Interview, `💰` Offer, `❌` Rejected, `🚫` Discarded, `⏭️` SKIP

Donde `{next_num}` es el número del report (mismo que `{{REPORT_NUM}}`).
