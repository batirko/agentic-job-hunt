 ...
 *
 * Runs over the live tracker and the archive, one file at a time. Duplicates
 * that span the two files are a merge bug, not a dedup one — merge-tracker.mjs
 * checks the archive before it opens a new live row, and verify-pipeline.mjs
 * reports any that slipped through.
 *
 * Run: node agentic-job-hunt/dedup-tracker.mjs [--dry-run]
 */

import { readFileSync, writeFileSync, copyFileSync, existsSync, mkdirSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import { splitRow, splitRowRaw, escapeCell } from './markdown-core.mjs';
import { trackerPaths } from './tracker-core.mjs';

const CAREER_OPS = dirname(fileURLToPath(import.meta.url));
// Both tracker files. Duplicates predate the split and exist inside the archive
// too, and a run that skipped it would report the tracker clean while leaving
// them there.
...
const STATUS_RANK = {
  ...
  'discarded': 0,
  'rejected': 1,
  // Terminal like rejected: if a duplicate row shows 'applied', that is a
  // genuine re-application and should win over a retired row.
  'ignored': 1,
  'ghosted': 1,
  'evaluated': 2,
  'applied': 3,
  ...
  for (let i = 0; i < companyEntries.length; i++) {
    ...
    if (bestStatus !== keeper.status) {
      ...
      if (lineIdx !== undefined) {
        const parts = splitRowRaw(lines[lineIdx]).map(s => s.trim());
        parts[6] = escapeCell(bestStatus);
