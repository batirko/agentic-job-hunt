#!/usr/bin/env node
/**
 * followup-cadence.mjs — Follow-up Cadence Tracker for agentic-job-hunt
 *
...
import { parseTracker, resolveTrackerPath, resolveReportPath, effectiveAppliedDate } from './tracker-core.mjs';
import { splitRow } from './markdown-core.mjs';

const CAREER_OPS = dirname(fileURLToPath(import.meta.url));
// Live tracker only, by design: a follow-up is only worth sending on an open
// application, and every archived row is closed.
...
// Status normalization lives in states-core.mjs, sourced from
// templates/states.yml (applications.md stores statuses as emoji).
// 'ignored' (👻) is deliberately absent: it is the terminus of this loop.
// Once applied_max_followups is exhausted an application goes 'cold' here,
// and `node retire-stale.mjs --apply` retires it out of the queue for good.
...
// --- Resolve report link, keeping the tracker-relative form for output ---
function reportLinkIfExists(reportField) {
  const match = String(reportField ?? '').match(/\]\(([^)]+)\)/);
  if (!match) return null;
  const fullPath = resolveReportPath(reportField, APPS_FILE);
  return fullPath && existsSync(fullPath) ? match[1] : null;
}
...
// --- Main analysis ---
function analyze() {
  const apps = parseTracker(APPS_FILE);
  if (apps.length === 0) {
  ...
  for (const app of apps) {
    ...
    // Cadence runs from the date the application was SENT, which is not always
    // the Date column (that is when the offer was evaluated).
    const appliedOn = effectiveAppliedDate(app.notes, app.date);
    const appDate = parseDate(appliedOn);
    if (!appDate) continue;
    ...
    const nextFollowupDate = computeNextFollowupDate(normalized, appliedOn, lastFollowupDate, followupCount);
    ...
    const reportPath = reportLinkIfExists(app.report);
