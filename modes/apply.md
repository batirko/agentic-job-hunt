...
Interactive mode for when the candidate is filling out an application form. It reads what's on the screen, loads previous context for the offer, and generates personalized answers for each form question.
...
4. LOAD      → Read full report + Section H (if it exists - drafted answers)
...
- Or say company + role for us to search for it

## Step 2 — Identify and search for context

1. Extract company name and role title from the page.
2. Search in `reports/` for the company name (Grep case-insensitive).
3. If there is a match → load the full report.
4. If there is Section H → load previous draft answers as a base.
...
- **Already answered in Section H** → adapt existing answer
...
5. **agentic-job-hunt proof point**: Include in "Additional info" if there is a field for it.
...
3. Suggest next step: `/job-hunt-contact` for LinkedIn outreach.
