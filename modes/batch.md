# Mode: batch — Mass Processing of Job Offers
...
| Error | Recovery |
|-------|----------|
| URL inaccessible | Worker fails → conductor marks `failed`, next one. |
| JD behind login | Conductor tries to read DOM. If it fails → `failed`. |
| Portal changes layout | Conductor reasons over HTML, adapts. |
| Worker crashes | Conductor marks `failed`, next one. Retry with `--retry-failed`. |
