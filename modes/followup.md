...
- `data/applications.md` — Application tracker, open rows. Follow-ups apply to open applications only, so the archive (`data/applications-archive.md`) is deliberately out of scope here
...
> "No active applications to follow up on. Apply to some roles first with `/job-hunt` and come back when they're aging."
...
- Suggest the user run `/job-hunt contacto {company}` to find the right person first
...
> - Retiring it to `👻 Ignored` — run `node retire-stale.mjs` to sweep every silent application at once (dry run; `--apply` to write)
> - Trying a different contact via `/job-hunt contacto`
> - Keeping in `✅ Applied` status but deprioritizing"

Do not mark it `🚫 Discarded`. That status means the candidate withdrew or the position closed, and neither happened here.
...
**To:** {email or "No contact found — run `/job-hunt contacto` first"}
...
|--------|----------------|------------|-------------|
| Applied | 7 days after application | Every 7 days | 2 (then cold → retire to `👻`) |
...
A cold application ends at `👻 Ignored`. `node retire-stale.mjs` sweeps every `✅` row that has been silent past the window (default 14 days) and retires it out of the follow-up queue.
