...
- `data/applications.md` — Application tracker, open rows
- `data/applications-archive.md` — Application tracker, closed rows. **Most of the signal lives here** — the rejections and the silences. `analyze-patterns.mjs` reads both automatically; if you read the tracker by hand, read both or the analysis reports on the applications still in flight and calls it a pattern
...
Before running analysis, check: do the two tracker files together have at least 5 entries with status beyond "Evaluated" (i.e., Applied, Responded, Interview, Offer, Rejected, Discarded, Ignored, SKIP)?
...
| Ignored | **No response** (applied, never heard back, retired) |
| SKIP, NO APLICAR | **Self-filtered** (user decided not to apply) |
| Evaluated | **Pending** (no action taken yet) |

**No response is kept separate from Negative on purpose.** "They said no" and "they never looked" are different signals: folding ghosting into the rejection rate hides how much of the funnel is silence rather than a decision. Both count toward tech-stack gap analysis, though, since a ghosted application is still a failed one.
